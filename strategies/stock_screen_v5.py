#!/usr/bin/env python3
"""全市场A股选股 v5 —— 技术面 + 基本面（财务因子）结合版。

v5 相对 v4 的核心变化：
  新增第 6 类「基本面」因子（财务维度），从纯技术面升级为技基结合：
    · 净利润增速（10分）：最新年报归母净利润 vs 上年同期
    · 营收增速（5分）：最新年报营业收入 vs 上年同期
    · 筹码集中度（10分）：最新股东户数 vs 前期，户数减少=筹码集中=加分
    · 资金关注度（5分）：近期是否上龙虎榜 + 净买入为正
  采用「两段式」：
    第一段：技术面（v4 完整逻辑）先筛出候选池
    第二段：仅对候选池拉财务因子做二次排序，避免全市场 5200+ 只拉财务太慢

综合分 = 技术分(0-100) + 财务分(0-30)，满分 130。
  输出保留 tech_score 与 fund_score 两个独立分数，便于技基对比。

财务接口（星耀数智 AmazingData InfoData）：
  · get_income      -> {code: DataFrame}，字段 REPORTING_PERIOD / REPORT_TYPE(4=年报)
                       / OPERA_REV(营收) / NET_PRO_EXCL_MIN_INT_INC(归母净利) / BASIC_EPS
...
用法：
  python stock_screen_v5.py            # 默认跑全市场（技术+财务）
  python stock_screen_v5.py --quick    # 快速模式（仅扫描成交额前800只）
  python stock_screen_v5.py --no-fund  # 只看技术面（等价 v4，跳过财务）
"""
import os
import sys
import json
...
def main():
    ...
    print(f"\n===== 通过筛选 {len(results)} 只，按综合分（技术+财务）排序 =====\n")
    ...
    with open("screen_result_v5.json", "w", encoding="utf-8") as f:
        json.dump(out, f, ensure_ascii=False, indent=2)
    print(f"\n结果已存 screen_result_v5.json（共 {len(results)} 只，Top{args.top} 见上）")
