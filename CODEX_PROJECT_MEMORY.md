...
- `watchlists/whole_market_watchlist.json` and `tools/whole_market_watch_report.py` implement the four-layer whole-market funnel: market gate -> sector breadth -> 3 deep research modules -> small stock set. The public module tags also include Mason-style research labels such as `顺大势逆小势 / 等待回踩 / 不追`, but never emit buy amounts, target prices, fixed positions, or orders. `.github/workflows/whole-market-watch-daily.yml` sends the main public QQ email after the A-share close and persists only public snapshots; it uses 15:40/15:50/16:00 Beijing backup cron attempts with same-day duplicate skipping because GitHub scheduled runs can occasionally be missed.
- Saved whole-market reports store return-window keys as strings after JSON serialization. `tools/whole_market_watch_report.py` must render both integer and string metric keys so manual `--resend-existing` emails do not lose 5日/20日/相对300 columns. The scheduled whole-market workflow gives the broad-market scan a longer 180-second budget to reduce 4700/5000-style partial coverage after close.
- The whole-market workflow has a manual `resend_saved_report` option for weekend/holiday review. It resends only the saved A-share trading-day report, does not refresh data or rewrite history, and refuses non-trading-day snapshots.
...
- GitHub Actions schedules are stored as UTC cron expressions, with comments documenting the intended Beijing time. Do not add `timezone: Asia/Shanghai` fields or write local Beijing hours directly in cron; the Python scripts still perform Asia/Shanghai trading-day/time guards. Current Beijing checkpoints: premarket brief 08:30 (`00:30 UTC`); intraday action card uses offset triggers at 09:37/10:07/10:37/11:07/11:37/13:07/13:37/14:07/14:37/14:57 to avoid :00/:30 scheduler peaks, while the script maps each run to the 09:30/10:00/10:30/11:00/11:30/13:00/13:30/14:00/14:30/14:55 checkpoints with at most 20 minutes of delay; market snapshots 09:10 and 15:35 (`01:10/07:35 UTC`); whole-market post-close retries 15:40/15:50/16:00 (`07:40/07:50/08:00 UTC`); weekly/monthly/quarterly/yearly reviews use Beijing 17:30 (`09:30 UTC`) with script-side guards for month-end and quarter-end. GitHub cron still does not support `L` month-end syntax, so month/quarter workflows run on candidate days and skip unless the Beijing date is actually the period end.
