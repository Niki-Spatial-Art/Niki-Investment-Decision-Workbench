...
- `tools/whole_market_watch_report.py` now emits a daily action-card table in the public market report, and `tools/export_daily_action_cards.py` can export the latest or historical card set to Markdown/CSV using the fixed schema `日期｜板块｜股票｜产业催化｜价格/成交证据｜反证条件｜结论（观察/复核）｜梅森标签`.
