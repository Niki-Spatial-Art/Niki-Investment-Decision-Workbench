...
def module_metrics(module: dict[str, Any], payload: dict[str, Any], benchmark_returns: dict[int, float | None], rules: dict[str, Any], mason_rules: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if len(active) < len(stocks):
        ...
        status = "观察：趋势或扩散未确认"
    if not (payload.get("market_gate_ok") if isinstance(payload, dict) else False):
        candidates = []
    return {
...
def build_report(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if gate.startswith("观察："):
        for module in modules:
            final_candidates.extend(module.get("backtest_candidates") or [])
    return {
...
def render_html(report: dict[str, Any]) -> str:
    ...
    final_rows = "".join("<tr>" + "".join(cell(v) for v in [x.get("code"), x.get("name"), x.get("signal_backtest", {}).get("reason"), x.get("signal_backtest", {}).get("horizons", {}).get("1", {}).get("observations", 0), x.get("signal_backtest", {}).get("horizons", {}).get("5", {}).get("net_return")]) + "</tr>" for x in report.get("final_candidates") or []) or "<tr><td colspan=5>0 只：市场闸门或回测阈值未通过</td></tr>"
